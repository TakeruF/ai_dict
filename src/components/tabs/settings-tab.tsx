...
> = {
  anthropic:  { model: "claude-sonnet-5",              input: "$2.00",  output: "$10.00", perSearch: "~$0.01",  freeTier: null,                 keyUrl: "console.anthropic.com" },
  gemini:     { model: "gemini-2.5-flash",             input: "$0.30",  output: "$2.50",  perSearch: "~$0.003", freeTier: "1,500 req/day",       keyUrl: "aistudio.google.com" },
  openai:     { model: "gpt-5.4-mini",                 input: "$0.75",  output: "$4.50",  perSearch: "~$0.005", freeTier: null,                 keyUrl: "platform.openai.com" },
  deepseek:   { model: "deepseek-v4-flash",            input: "$0.14",  output: "$0.28",  perSearch: "~$0.0005", freeTier: null,                keyUrl: "platform.deepseek.com" },
  ...
  return (
    <div className="flex flex-col gap-6">
        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-col gap-2">
            ...
            <p className="text-[11px] text-muted-foreground leading-relaxed">
              {isEn
                ? "Stored only in your browser's localStorage. Requests go straight from this device to the provider — we run no server."
                : isZh
                  ? "密钥仅保存在浏览器本地。请求由本设备直接发送给服务商，我们不运行任何服务器。"
                  : "APIキーはこの端末のLocalStorageにのみ保存されます。リクエストは端末から各プロバイダーへ直接送信され、当方はサーバーを一切持ちません。"}
            </p>
          </div>
        </div>
      </Section>
      ...
      <div className="text-center text-[11px] text-muted-foreground space-y-1 pb-4">
        ...
        <p>Powered by Claude / Gemini / GPT-5.4 / DeepSeek</p>
      </div>
    </div>
  );
}
